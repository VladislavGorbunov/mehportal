...
@section('content')
    <h2 class="fs-4">Заявки на Premium от заказчиков</h2>
        <x-site.message />
        
         @foreach ($requests as $request)
        <div class="col-12 mt-3 mb-3 border rounded p-3">
            <p>ID исполнителя: {{ $request->customer_id }}</p>
            ...
            <a href="{{ Route('premium-customer-request-delete', ['id' => $request->id]) }}" class="btn btn-blue"><small>Удалить заявку</small></a>
        </div>
        @endforeach 
