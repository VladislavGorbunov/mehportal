            ...
            <div class="col-12 col-md-8">
                ...
                <div class="mb-3 mt-2 d-flex flex-wrap">
                    @foreach ($order['services'] as $service)
                        @if ($regionSlug !== '')
                            <div class="services-list me-2 mb-2 d-flex align-items-center"><a href="/{{ $regionSlug }}/orders/service/{{ $service->slug }}">{{ $service->title }}</a></div>
                        @else
                            <div class="services-list me-2 mb-2 d-flex align-items-center"><a href="{{ $regionSlug }}/orders/service/{{ $service->slug }}">{{ $service->title }}</a></div>
                        @endif
                    @endforeach
                ...
                <div class="mb-0 mt-4 d-flex flex-column flex-md-row align-items-start">
                    ...
                    <a href="/order/{{ $order['order_id'] }}" class="btn btn-more col-12 col-md-3 mb-2 me-3 d-flex align-items-center justify-content-center" target="_blank"><i class="bi bi-caret-right-fill"></i> Подробнее о заказе</a>
                    ...
                    @else
                        <a class="btn btn-file-download-noactive mb-2 col-12 col-md-3 d-flex align-items-center justify-content-center"><i class="bi bi-cloud-arrow-down"></i> Файлы отсутствуют</a>
                    @endif
                </div>
            </div>
        </div>
